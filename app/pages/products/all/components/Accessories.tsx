...
const Accessories = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:8080/accessories-api/products');
  ...
  return (
    <div className=' '>
      ...
      modules={[Pagination]}
      className="mySwiper w-full p-10 m-10"
      breakpoints={{
        320: {
          slidesPerView: 1,
          spaceBetween: 10,
        ...
        1024: {
          ...
          products.map((product) => (
                <SwiperSlide  key={product.id} className='text-start p-2 m-2 mb-10 w-max h-80 rounded-md bg-slate-500'>
                  ...
                  <Link className='font-bold p-2 m-2' href={`/products/category/${product.id}`}>See More &rarr;</Link>
                </SwiperSlide>
